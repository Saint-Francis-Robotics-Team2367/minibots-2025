...
def countdown():
    global remaining_time, teleop_running, teleop_paused
    while teleop_running and remaining_time > 0:
        if teleop_paused:
    ...
    if teleop_running and remaining_time == 0:
        set_status("standby")
        teleop_running = False
        print("Teleop ended, switched to standby")

def start_teleop_timer():
    global teleop_running, teleop_paused, teleop_thread
    if not teleop_running:
        set_status("teleop")
        teleop_running = True
        teleop_paused = False
        teleop_thread = Thread(target=countdown, daemon=True)
        teleop_thread.start()
    elif teleop_paused:
        # Resume if paused
        set_status("teleop")
...
def stop_teleop_timer():
    global teleop_paused
    if teleop_running:
        teleop_paused = True
        set_status("standby")
        print("Teleop paused")

def reset_teleop_timer():
    global remaining_time, teleop_running, teleop_paused
    teleop_running = False
    teleop_paused = False
    remaining_time = 120
...
tk.Button(root, text="Start Teleop (2 min)", width=20, command=start_teleop_timer).pack(pady=2)
tk.Button(root, text="Stop Timer", width=20, command=stop_teleop_timer).pack(pady=2)
tk.Button(root, text="Reset Timer", width=20, command=reset_teleop_timer).pack(pady=2)
